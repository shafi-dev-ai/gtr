...
export const CreateForumPostScreen: React.FC = () => {
  ...
  const uploadForumPhotos = async (): Promise<string[]> => {
    if (!user || photos.length === 0) return [];
    const urls: string[] = [];
    for (let i = 0; i < photos.length; i++) {
      const { fileData } = await prepareImageForUpload(photos[i].uri);
      const filePath = `${user.id}/forum/${Date.now()}-${i}.jpg`;
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from('forum-images').getPublicUrl(filePath);
      urls.push(publicUrl);
    }
    return urls;
  };
  ...
  const handlePublish = async () => {
    if (!user) {
      Alert.alert('Sign in required', 'Please sign in to post in the forum.');
      return;
    }
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow}>
            {GTR_MODEL_OPTIONS.map((option) => {
              const active = selectedModelKey === option.value;
              return (
                <TouchableOpacity
                  key={option.value}
                  style={[styles.chip, active && styles.chipActive]}
                  onPress={() => setSelectedModelKey(option.value)}
                >
        ...
        <View style={styles.section}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Share your build, question, or event recap"
        ...
        <View style={styles.section}>
          ...
          <TextInput
            ...
            placeholder="Tell the GT-R community what's happening..."
            placeholderTextColor="#808080"
            multiline
            value={content}
